	...
	mc.connect("mongodb://localhost:27017", function(err, dbs){
		...
		app.post("/updateLinkInfo", function(req, res){
			...
			app.post("/login", function(req, res){
				...
				col.find({username:req.body.uname, password:req.body.passwd})
					.project({"links":0, "password":0, "phone":0, "totalcount":0}).toArray(function(err, docs){
					if(docs.length==0){
			...
			app.get("/getuserLinks/:uid", function(req,res){
				col.find({_id:ObjectId(req.params.uid)}).project({"links":1}).toArray(function(err, docs){
					res.json(docs);
				});
			});

			app.get("/userProfile/:uid", function(req,res){
				col.find({_id:ObjectId(req.params.uid)}).project({"links":0}).toArray(function(err, docs){
					res.json(docs);
				});
			});

			// app.get("/getclicks/:uid/:lid", function(req,res){
			// 	col.find({_id:ObjectId{req.params.uid}, 
			// 	})
			// });
